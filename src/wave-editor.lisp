;;;; wave-editor.lisp - wave editor main gui code.
...
(defclass wave-editor ()
  ((bdef :initarg :bdef :initform nil :reader wave-editor-bdef)
...
(defmethod (setf wave-editor-bdef) (value (wave-editor wave-editor))
  (setf (slot-value wave-editor 'bdef) value)
...
(defun (setf selection-start) (frame &optional (wave-editor *wave-editor*))
  ...
  (let ((length (bdef-length (wave-editor-bdef wave-editor))))
    ...
    (prog1
      ...
      ;; (print 'selection-start-1)
      (update-sel-rect wave-editor)
      ;; (print 'selection-start-2)
      ;; (update-status-text wave-editor)
      ;; (print 'selection-start-3)
      )))
...
(defun load-object (&optional (wave-editor *wave-editor*))
  (let ((file (get-open-file)))
    (format t "Selected ~S.~%" file)
    (let ((bdef (bdef file)))
      (setf (wave-editor-bdef wave-editor) bdef))))
...
(defun wave-editor-about ()
  (do-msg (format nil "wave-editor v0.1, by modula t. worm.~%A simple audio file editor.") :title "wave-editor: About"))
...
(defmethod make-window ((wave-editor wave-editor))
  (with-slots (bdef window (wave-editor::scrolled-canvas scrolled-canvas) canvas status) wave-editor
...
(defun wave-editor (&optional wave &key)
  (setf *wave-editor* (make-instance 'wave-editor)
        (wave-editor-bdef *wave-editor*) (etypecase wave
                                           (bdef wave)
                                           (string (bdef wave))
                                           (null nil)
                                           (symbol (find-bdef wave))))
  (make-window *wave-editor*)
  *wave-editor*)

(defun launch-wave-editor (&rest args)
  (bt:make-thread (fn (apply #'wave-editor args))
